...
public interface Application {
    static void main(String[] args) {
        ...
        var http = HttpServer.create()
                ...
                .route(routes -> routes
                        .post("/sign", (q, r) -> r.send(q.receive().aggregate()
                                ...
                                .flatMap(buf -> {
                                    ...
                                    var dfn = buf.readIntLE();
                                    var dateFont = dfn > 0 ? buf.readSlice(dkn).toString(StandardCharsets.UTF_8) : null;
                                    ...
                                    return serv.sign(file, sign, signKey, seal, sealKey, date, dateKey, dateFont, form);
                                })))
                        .get("/fonts",(q,r)->r.sendString(Mono.just(String.join("\n", serv.fonts()))))
                );
        http.warmup().block();
        http.bindNow()
                .onDispose()
